import { itemsForModule } from '@/content';
...
/**
 * Build a drill session for a module: items due for review first (oldest
 * due leading), then unseen items, capped at SESSION_SIZE.
 */
export function buildSession(moduleId: string, track: Track, now: number): Item[] {
  const pool = itemsForModule(moduleId, track);
